#!/usr/bin/env python3
"""
Script d'entraînement pour Bio_ClinicalBERT optimisé pour le RECALL (sensibilité).

Version alternative de train_model.py qui sélectionne le meilleur modèle
selon le recall_macro plutôt que le f1_macro.

En contexte médical, le recall est souvent préférable car un faux négatif
(rater une maladie) est généralement plus grave qu'un faux positif.

Usage:
    .venv/bin/python scripts/training/train_model_recall.py "panic disorder,pneumonia,cystitis"
"""
...
app = typer.Typer(help="Train Bio_ClinicalBERT for disease classification (RECALL optimized)")
console = Console()

# Default paths
DEFAULT_DATA_PATH = Path(__file__).parent.parent / "data" / "diseases_symptoms.csv"
DEFAULT_OUTPUT_DIR = Path(__file__).parent.parent / "models" / "symptom_classifier_recall"
...
def train(
    ...
    """
    Train Bio_ClinicalBERT for disease classification (RECALL optimized).

    This version selects the best model based on recall_macro instead of f1_macro,
    which is more appropriate for medical contexts where missing a disease
    (false negative) is worse than a false alarm (false positive).
    """
    # ========================================================================
    # 1. Setup
    # ========================================================================
    console.print("\n[bold cyan]Bio_ClinicalBERT Training Pipeline (RECALL optimized)[/bold cyan]")
    console.print("=" * 60)
    ...
    training_args = TrainingArguments(
        ...
        metric_for_best_model="recall_macro",  # <-- RECALL instead of F1
    ...
    console.print("[yellow]Best model selected by: recall_macro[/yellow]")
    console.print("=" * 60)
    ...
    summary_table.add_row("Test Recall", f"{test_results['eval_recall_macro']:.4f}")
